...
# Canonical 60Hz timings from CEA-861-D / DMT standards.
# (pclk_khz, hblank, hso, hsp, vblank, vso, vsp, h_pos, v_pos)
_CEA_60HZ_MODES = {
    (1280,  720,  60): ( 74250,  370, 110,  40,  30,  5,  5,  True,  True),  # CEA Mode 4
    (1920, 1080,  60): (148500,  280,  88,  44,  45,  4,  5,  True,  True),  # CEA Mode 16
    (3840, 2160,  60): (594000,  560, 176,  88,  90,  8, 10,  True,  True),  # CEA Mode 95
}

# Resolution → refresh-rate list. Mainstream gaming refresh rates only.
# Cuts from Sony's full PS5 spec list:
#   * 24Hz / 50Hz film + PAL modes — advertised by the PS5 firmware but use
#     unusual timings the Linux kernel doesn't always handle cleanly.
#   * 4K@120 — needs a CTA-861 extension block (pclk 1118 MHz exceeds the
#     655 MHz DTD-field cap), and extensions are exactly what trips the
#     MN864739 converter.
...
def build_synthetic_edid(width, height, refresh_hz, monitor_name=None,
    ...
    # ── Byte 126: extension count = 1 (we append one CEA-861 block) ──────
    e[126] = 1
    ...
    # ── Bytes 128..255: minimal CEA-861 extension block (audio-only) ─────
    # This is the smallest valid CTA extension that tells the kernel "HDMI
    # audio is supported" without advertising any high-bandwidth video
    # features (HDR, BT.2020, VRR, alternate refresh modes) that would trip
    # the MN864739 DP→HDMI converter. Without this block the kernel's
    # snd_hda_intel codec sees no audio capability and PulseAudio reports
    # "Dummy Output".
    ext = bytearray(128)
    ext[0] = 0x02                  # CTA-861 extension tag
    ext[1] = 0x03                  # revision 3
    ext[2] = 0x04                  # no data blocks; DTD region starts at 4
    ext[3] = 0x40                  # bit 6 = Basic Audio supported
    # bytes 4..126 stay 0 (no data blocks, no DTDs, just padding)
    s2 = sum(ext[:127]) & 0xFF
    ext[127] = (256 - s2) & 0xFF
    return bytes(e) + bytes(ext)
...
def bake_edid_into_usb(usb_path, edid_name, edid_full_bytes, strip=True):
    ...
    full_path = edid_dir / f"{edid_name}-full.bin"
    if not full_path.exists():
        full_path.write_bytes(edid_full_bytes)
        log.append(f"wrote {full_path} ({len(edid_full_bytes)} B, reference)")
...
class WizardApp:
    ...
    def on_bake_universal(self):
        """Bake the synthetic SAFE_1080P_EDID — no display scan needed.
        Works first-try on essentially any display. The user can re-bake
        with their actual monitor's EDID later for native resolution."""
        if not self._selected_usb:
            messagebox.showinfo("Pick a USB", "Click Detect or Browse first.")
            return
        ok = messagebox.askyesno(
            "Bake Universal Safe Mode?",
            f"Bake the universal 1920x1080@60Hz safe-mode EDID into:\n"
            f"  {self._selected_usb}\n\n"
            f"This is the 'just make it work' option — no display scan needed. "
            f"Every TV/monitor supports 1080p60 SDR as a fallback, so the PS5 "
            f"Linux display WILL come up first try, regardless of which display "
            f"is plugged in.\n\n"
            f"After the screen works, you can re-bake with your actual monitor's "
            f"EDID (the per-monitor buttons) to get its native resolution.")
        if not ok:
            return
        try:
            for ln in bake_edid_into_usb(
                self._selected_usb, "universal-safe-1080p60",
                SAFE_1080P_EDID, strip=False):
                self._log("bake: " + ln)
            messagebox.showinfo(
                "Done",
                "Universal Safe Mode baked. Eject the USB and move it to the PS5.\n\n"
                "After boot, your display should come up at 1080p60 SDR.")
        except Exception as e:
            self._log(f"bake: ERROR {e}")
            messagebox.showerror("Bake failed", str(e))
